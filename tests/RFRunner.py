...
try:
    if len(sys.argv) > 3:
        ...
        specified_browser = str(sys.argv[3])
    else: 
        includetag = str(sys.argv[1])
        excludetag = str(sys.argv[2])
        specified_browser = ' '
...
try:
    ...
    if specified_browser in (' ', ''):
        browser = ' --variable BROWSER:ff'
    else:
        browser = ' --variable BROWSER:' + specified_browser + ' '
except OSError, error_msg:
    print "Something has gone wrong with the browser variable..."
    print error_msg
