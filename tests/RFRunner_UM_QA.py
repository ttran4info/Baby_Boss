...
try:
    umhosttag = str(sys.argv[3])
    if umhosttag in (' ', ''):
        umhost = ' '
    else:
        umhost = ' --variable QA_USER_MANAGEMENT:' + umhosttag + ' '
except OSError, error_msg:
    print "No umhost given, defaulting to no exclude..."
    umhost = ' '
    print error_msg

try:
    umporttag = str(sys.argv[4])
    if umporttag in (' ', ''):
        umport = ' '
    else:
        umport = ' --variable QA_USER_MANAGEMENT_PORT:' + umporttag + ' '
except OSError, error_msg:
    print "No umport given, defaulting to no exclude..."
    umport = ' '
    print error_msg
